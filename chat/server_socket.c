...
int main(void)
{
	...
	pr_debug("bind socket %d\n", sockfd);

	pr_debug("listen start %d\n", sockfd);
	if (listen(sockfd, BACKLOG) == -1) {	 /*错误检测*/
		perror("listen");
		exit(1);
	}
	pr_debug("listen end %d\n", sockfd);

	sin_size = sizeof(struct sockaddr_in);
	/*accept 会等待client去connect*/
...
/*接收从客户端传来的用户名*/	
	...
	if((fp=fopen("name.txt","a+"))==NULL)	
		{		
			printf("can not open file name.txt,exit...\n");		
			return -1;	
		}
	if((fs_record=fopen("s_record.txt","a+"))==NULL)	
		{		
			printf("can not open file s_record.txt,exit...\n");		
			return -1;	
		}
		...
		if(FD_ISSET(STDIN, &rfd_set))
		 {
			...
			if (strcmp("record", send_str) == 0) {	  /*打开聊天记录*/
				if((f_read=fopen("s_record.txt","r"))==NULL)	
				{		
					printf("can not open file s_record.txt,exit...\n");		
					return -1;	
				}
			...
			printf("        ( %4d年%d月%2d日  ",(1900+p->tm_year),(1+p->tm_mon),p->tm_mday);
			printf("%s %2d:%2d:%2d )\n",wday[p->tm_wday],p->tm_hour,p->tm_min,p->tm_sec);
			fprintf(fs_record,"%s\n",send_str);
			fprintf(fs_record,"        ( %d年%d月%2d日  ",(1900+p->tm_year),(1+p->tm_mon),p->tm_mday);
			fprintf(fs_record,"%s %2d:%2d:%2d )\n",wday[p->tm_wday],p->tm_hour,p->tm_min,p->tm_sec);
		}
/*判断是否已将新建的描述符加到seletct()监视的读的文件描述符集合中*/
		if (FD_ISSET(client_fd, &rfd_set))
         ...
         printf("        ( %d年%d月%2d日  ",(1900+p->tm_year),(1+p->tm_mon),p->tm_mday);
			...
			fprintf(fs_record,"%s:%s\n",buff,buf);
         fprintf(fs_record,"        ( %d年%d月%2d日  ",(1900+p->tm_year),(1+p->tm_mon),p->tm_mday);
			fprintf(fs_record,"%s %2d:%2d:%2d )\n",wday[p->tm_wday],p->tm_hour,p->tm_min,p->tm_sec);
			fprintf(fs_record,"Server: ");
			fflush(stdout);
		}
