...
ADB=adb
GDB=${GDB:-prebuilt/$(uname -s | tr "[[:upper:]]" "[[:lower:]]")-x86/toolchain/arm-eabi-4.4.3/bin/arm-eabi-gdb}
B2G_BIN=/system/bin
...
if [ "$1" = "attach"  -a  -n "$2" ] ; then
   ...
   #echo "first1 B2G_BIN= $B2G_BIN"
   if [[ $B2G_BIN != /system/bin* ]]
   then
